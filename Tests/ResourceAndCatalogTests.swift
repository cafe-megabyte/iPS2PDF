...
final class ResourceAndCatalogTests: XCTestCase {
    func testBundledResourceCountsAndSanitization() throws {
        ...
        for url in joboptions {
            let document = try LosslessJoboptionsDocument(data: Data(contentsOf: url))
            XCTAssertTrue(document.keys.isDisjoint(with: removedKeys), url.lastPathComponent)
            XCTAssertEqual(document.value(forKey: "AlwaysEmbed"), .array([]), url.lastPathComponent)
            XCTAssertEqual(document.value(forKey: "NeverEmbed"), .array([]), url.lastPathComponent)
        }
        for url in profiles {
