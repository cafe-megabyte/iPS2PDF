...
int main(int argc, char** argv) {
    ...
    if (!metadata && argc != 6) {
        std::fprintf(stderr, "usage: pdf-processing input output level monochrome threshold\n       pdf-processing metadata input output preserve_conformity\n"); return 2;
    }
    ...
    const int status = metadata ? ips2pdf_pdf_remove_metadata(argv[2], argv[3], nullptr, std::atoi(argv[4]), control, &result) :
        ips2pdf_pdf_compress(argv[1], argv[2], nullptr, std::atoi(argv[3]), std::atoi(argv[4]), std::atoi(argv[5]), control, &result);
