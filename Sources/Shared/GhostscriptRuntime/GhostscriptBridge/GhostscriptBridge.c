...
{
    *file = NULL;
    if (strcmp(filename, kDescriptorInputName) == 0) {
...
)
{
    ...
    char ghostscript_include[(PATH_MAX * 2) + 8];
    ...
    if (ghostscript_resource_directory == NULL || ghostscript_resource_directory[0] == '\0') {
        return_code = -1;
        goto descriptor_finished;
    }
    snprintf(resource_directory, sizeof(resource_directory), "%s/Resource/", ghostscript_resource_directory);
    snprintf(init_directory, sizeof(init_directory), "%s/Resource/Init", ghostscript_resource_directory);
    snprintf(generic_resource_option, sizeof(generic_resource_option), "-sGenericResourceDir=%s", resource_directory);
    snprintf(ghostscript_include, sizeof(ghostscript_include), "-I%s:%s", init_directory, ghostscript_resource_directory);
    ...
    if (last_page > 0) {
        snprintf(last_page_option, sizeof(last_page_option), "-dLastPage=%d", last_page);
        arguments[argument_count++] = last_page_option;
    }
    ...
    if (!validation_only) {
        return_code = descriptor_run_file(instance, kDescriptorInputName, &capture);
